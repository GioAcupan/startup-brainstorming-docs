...
team_idea_seed: {
  ...
  challenge_outcome: 'adopt_as_seed' | 'refine_and_adopt' | 'reject';
  refined_text: string | null;      // populated only when outcome is 'refine_and_adopt'
...
type PromptDefinition = {
  ...
  context_selectors: ContextSelector[];   // §4.3
  vault_query_hints: VaultQueryHint[] | null;  // auto-suggest Vault entries
  task_specification_template: string;    // string with {placeholder} slots filled from PCD
...
// Step 3a — gates the seed before it influences ideation. Always Grill-Me.
{
  ...
  context_selectors: [/* see §4.3 — includes user-supplied seed text passed at gen time */],
  vault_query_hints: null,
  task_specification_template: '...stress-test the seed against the brief, problem analysis, and crowded space; emit a verdict: adopt_as_seed | refine_and_adopt | reject...',
...
// Step 4a — stress-tests scope before the three Phase 4 documents are generated.
{
  ...
  target_intake_schema_path: 'sections.chosen_direction.data',  // adjustments amend chosen_direction
...
The role assignment and task specification strings are abbreviated above for readability. The full content per prompt is authored in the prompts module (`frontend/src/lib/prompts/templates/`) in the same style as existing prompts. The `task_specification_template` for `phase_3_divergent_candidates` carries two integer placeholders, `{candidate_count_min}` (default 3) and `{candidate_count_max}` (default 5), filled at generation time from settings.
...
Call schemaToOutputTemplate(target_intake_schema_path)
    │
    ▼
Assemble final prompt: preamble + role + context + (task spec OR Grill-Me task spec) + template
...
If Grill-Me mode is active, also surface an "Export Handoff Package" button:
  ...
  without Startup managing live chat history.
    │
    ▼
Log generation in Phase Gate Log (prompt_id, timestamp, included_vault_ids, grill_me_mode)
```
...
```ts
{
  ...
  extraction_prompt_template:
    ...
    'challenge_outcome must be exactly one of: "adopt_as_seed", "refine_and_adopt", "reject". ' +
    'refined_text is required only when challenge_outcome is "refine_and_adopt"; otherwise null.',
...

{
  ...
  extraction_prompt_template:
    ...
    'distinguishing_factors array. Generate a fresh ULID for each candidate.id during extraction. ' +
    'Set is_remix=false and remix_source_ids=[] for all extracted candidates — remixes are produced ' +
    'in-app via the candidate-review UI, not by the LLM.',
...

{
  ...
  // The Resolved Summary lists scope adjustments. The intake produces a structured
  // patch the user reviews via the standard diff preview (§6.5) before commit.
  ...
  extraction_prompt_template:
    ...
    'and intended scope. Each adjustment names the field path it touches (within ' +
    'chosen_direction.data, scope_definition.data, or product_brief.data) and the proposed new ' +
    'value. Adjustments are surfaced to the user for per-item confirm/dismiss before commit.',
  required_fields: ['adjustments'],
  optional_fields: ['unresolved_branches'],
  confidence_threshold: 0.7,
}

{
  ...
  extraction_prompt_template:
    ...
    'question, recommended_answer, and fallback_answer; associated_persona is optional. ' +
    'On commit, the user chooses (in the diff preview UI) whether the result REPLACES or ' +
    'SUPPLEMENTS the existing qa_prep_document — see §6.5 for replace-vs-merge semantics.',
...
`chosenDirectionAdjustmentsSchema` is a small auxiliary JSON Schema fragment defined alongside the intake registry — it shapes a list of `{ field_path, proposed_value, rationale }` records. The extraction is **not** committed directly; each adjustment passes through the standard diff preview (§6.5) and the user confirms or dismisses each before any write to `chosen_direction`, `scope_definition`, or `product_brief`. This is what the workflow doc means by "you confirm or dismiss each adjustment" — implementation-side, it's the diff preview with one row per adjustment.
...
function processIntake(result: FlashResult, def: IntakeDefinition): IntakeOutcome {
  ...
  if (missingRequired.length === 0 && lowConfidenceFields.length === 0) {
...
Some required fields missing OR some fields below confidence threshold. The system shows the same diff preview, but with the missing/low-confidence fields highlighted and inline-editable. The user fills gaps manually before confirming. This is the most common case for messy paste-back; designing for it is essential.

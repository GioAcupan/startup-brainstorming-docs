...
*Version 1.2 — YELLOW audit fixes (Adjustment shape, Rubric Mapping at M1, Phase 4 ordering), 2026‑05‑01*
> Companion docs: `pcd_schema.json`, `default_judging_rubric.md`, `project_context.mdc`
...
**Recent schema extension (v1.5 RED audit + v1.2 YELLOW audit).** The `event_type` enum in `pcd_schema.json` is extended with the following values: `prompt_generated`, `grill_me_session_initiated`, `grill_me_session_resolved`, `grill_me_adjustment_dismissed`, `phase_4_design_grill_skipped`. Additionally, a new optional `payload` field (type `object` or `null`, `additionalProperties: true`) is added to `PhaseGateLogEntry` to carry event‑specific structured data. These additions ship as a single ~15‑minute pre‑Milestone‑1 patch to `pcd_schema.json`; they are additive (no rename, no retype, no removal) and require no migration since no projects exist yet.   
...
Vault entries are referenced by ID, never copied into the PCD. This means a Vault entry can be edited later without retroactively mutating completed projects. The PCD records *which* Vault entries informed it; the Vault entries themselves remain Vault‑side.  
...
```ts
external_context_notes: SectionEnvelope<string>;  // data is the Markdown body itself,
                                                  // not an object wrapping it. "" until first save.
```

Wrapped in the standard `SectionEnvelope`, but with `data` typed as a bare string rather than an object — see `Section_ExternalContextNotes` in `pcd_schema.json`, which declares `data: { type: "string" }`. `is_populated` becomes `true` on first non‑empty save. `is_stale` is never set by the staleness algorithm (§5.3) — the dependency graph entry is empty.  

### 2.7 `createEmptyPCD()` factory  

The `createEmptyPCD` function in `frontend/src/lib/pcd/createEmptyPCD.ts` is the single entry point that produces a fresh, schema‑valid `PCD` instance. It is called once per project, by the New Project Setup screen’s "Create Project" handler (§9.4), after the form has been submitted, the Vault auto‑query has run, and — if the user pasted official judging criteria — the Rubric Mapping Intake has resolved. Its output is then encrypted and written to the `projects` Dexie table.  

No other code path constructs a PCD. All future state evolution happens by mutating an existing PCD (intakes, the Section Editor, reconciliation). The factory is the only place where the initial shape is established; if its shape is wrong, every downstream invariant about field presence becomes brittle.  

#### Signature  

```ts
import type { PCD } from './schema';
import type {
  HackathonProfileData,
  ActiveJudgingRubricData,
  VaultReferences,
} from './schema';
import { ulid } from 'ulid';
import { validatePCD } from './validate';

export function createEmptyPCD(input: {
  project_name: string;
  hackathon_profile: HackathonProfileData;
  default_rubric: ActiveJudgingRubricData;
  vault_references?: VaultReferences;
  now?: () => string;     // testability: defaults to () => new Date().toISOString()
  newId?: () => string;   // testability: defaults to ulid
}): PCD;
```

**Inputs.**  

- `project_name` — the user‑chosen project name from New Project Setup. The form defaults this to the hackathon name, but the field is independent so a later rename does not disturb `hackathon_profile.data.hackathon_name`.  
- `hackathon_profile` — the typed shape of `Section_HackathonProfile.data`, populated from the form. Required because Phase 1 is populated at t=0 by definition; an empty profile would make the project meaningless.  
- `default_rubric` — the typed shape of `Section_ActiveJudgingRubric.data`. Required because the schema enforces `criteria.minItems: 1`; the rubric must be present from t=0. The caller is responsible for resolving this value: when the user did not paste official criteria, the value is the parsed default rubric (from `parseDefault.ts`); when they did, it is the resolved output of the Rubric Mapping Intake, with whichever `rubric_source` the user selected (`'default'`, `'official'`, or `'official_with_default_supplement'`).  
- `vault_references` — optional; defaults to `{ judge_organizer_ids: [], winning_solution_ids: [], boilerplate_component_ids: [] }`. Always present (with empty arrays if no auto‑query matched) so downstream Vault writers never have to null‑check the parent object.  
- `now`, `newId` — testability hooks. Production code uses the defaults.  

**Output.** A `PCD` instance that has been validated against `pcd_schema.json` and passes. The factory always validates; on failure it throws (the failure represents a bug, not a recoverable user error). This guards against schema drift: the day someone edits `pcd_schema.json` without updating the factory, the next test run breaks loudly instead of producing a malformed PCD that fails much later at first persistence.  

#### 2.7.1 Initial state of every section  

Each row below is the SectionEnvelope produced for that section at fresh‑PCD init. Sections marked `is_populated: true` carry real Phase 1 content from inputs; the rest are shaped placeholders.  

| Section                             | `is_populated` | `populated_by_phase` | `data` initial value                       |  
|-------------------------------------|----------------|----------------------|---------------------------------------------|  
| `hackathon_profile`                 | `true`         | `1`                  | from input                                  |  
| `judge_intelligence`                | `false`        | `1`                  | `{}`                                        |  
| `active_judging_rubric`             | `true`         | `1`                  | from input                                  |  
| `problem_analysis`                  | `false`        | `2`                  | `{}`                                        |  
| `competitive_landscape`             | `false`        | `2`                  | `{ premortem_completed: false }`            |  
| `user_research`                     | `false`        | `2`                  | `{ research_status: 'not_started' }`        |  
| `ideation_record`                   | `false`        | `3`                  | `{ archetypes_enforced: true }`             |  
| `chosen_direction`                  | `false`        | `3`                  | `{}`                                        |  
| `scope_definition`                  | `false`        | `4`                  | `{}`                                        |  
| `technical_direction`               | `false`        | `4`                  | `{}`                                        |  
| `product_brief`                     | `false`        | `4`                  | `{}`                                        |  
| `technical_implementation_document` | `false`        | `4`                  | `{}`                                        |  
| `demo_script_and_backup_plan`       | `false`        | `4`                  | `{}`                                        |  
| `pitch_brief`                       | `false`        | `6`                  | `{}`                                        |  
| `judge_objection_map`               | `false`        | `6`                  | `{}`                                        |  
| `qa_prep_document`                  | `false`        | `6`                  | `{}`                                        |  
| `debrief_record`                    | `false`        | `7`                  | `{}`                                        |  
| `external_context_notes`            | `false`        | `1`                  | `""` (bare string per §2.6, not an object)  |  

For every section, the remaining envelope fields are constant: `is_stale: false`, `stale_reason: null`, `last_updated: now()`, `extraction_confidence: null`, `user_overrides: []`.  

Three sections need their `data` populated to non‑empty values to satisfy schema requirements or to express semantically meaningful initial state without relying on Ajv default‑application:  

- **`competitive_landscape.data.premortem_completed: false`** — required by the schema (`Section_CompetitiveLandscape` has `required: ["premortem_completed"]`). False is the correct initial value; the Phase 2 Competitive Pre‑mortem intake flips it to true.  
- **`user_research.data.research_status: 'not_started'`** — schema‑optional but semantically required for downstream code that reads this field. The user research intake updates it to `'in_progress'`, `'complete'`, or `'skipped_no_user_access'` as appropriate.  
- **`ideation_record.data.archetypes_enforced: true`** — see the rationale paragraph in §2.6. Set explicitly because Ajv runs without `useDefaults: true`. The schema’s `"default": true` literal is informational documentation, not behaviorally enforced.  

#### 2.7.2 Initial state of the root  

| Field                  | Initial value                                                                                                       |  
|------------------------|---------------------------------------------------------------------------------------------------------------------|  
| `schema_version`       | `'1.0.0'` (matches the `const` in the schema)                                                                       |  
| `project_id`           | `newId()` — a fresh ULID                                                                                            |  
| `project_name`         | from input                                                                                                          |  
| `created_at`           | `now()`                                                                                                             |  
| `archived_at`          | `null`                                                                                                              |  
| `current_phase`        | `1`                                                                                                                 |  
| `phase_4_gate_crossed` | `false`                                                                                                             |  
| `vault_references`     | from input, or `{ judge_organizer_ids: [], winning_solution_ids: [], boilerplate_component_ids: [] }` if not given  |  
| `sections`             | per the table in §2.7.1                                                                                             |  
| `phase_gate_log`       | `[]`                                                                                                                |  
| `addenda`              | `[]`                                                                                                                |  

The factory does not append a synthetic "project_created" event to `phase_gate_log`. The log’s `event_type` enum (see schema lines 175‑188) does not include such an event, and the root‑level `created_at` is the system of record for project creation time. This is consistent with the discipline of not adding vocabulary on architectural‑completeness instinct — when a real reason emerges to need a `project_created` event, the enum can be extended then.  

#### 2.7.3 The `populated_by_phase` semantic for unpopulated sections  

The schema requires `populated_by_phase` as a non‑null integer 1‑7 on every SectionEnvelope, including sections that have not yet been populated. The factory sets this field to **the canonical phase that will populate the section** as listed in §2.3, not to a sentinel.  

This is a forward‑looking semantic: "this section, when populated, is expected to be populated by Phase X." The alternative — making `populated_by_phase` nullable — would be a schema‑level change and is not pursued. The forward‑looking convention is consistent with how unpopulated sections are already treated elsewhere (e.g., `last_updated: now()` is also "the time this empty envelope was created" rather than "the time content was last written into it").  

Two sections deserve specific notes:  

- **`user_research`** is set to `populated_by_phase: 2` because §2.3 lists "2 or 3" with Phase 2 as the typical case. If the user research intake actually runs in Phase 3, the intake updates `populated_by_phase` to `3` at commit time. This is the only section whose `populated_by_phase` may legitimately change at first population.  
- **`external_context_notes`** is set to `populated_by_phase: 1`. The section’s design (§2.3) does not bind it to any phase — it can be edited from project creation onward. On first non‑empty save (when `is_populated` flips to `true`), `populated_by_phase` is updated to the current phase at that moment. The field thus records *when the user first authored notes*, since the section has no other phase association.  

#### 2.7.4 Function structure  
...
export function createEmptyPCD(input: {
  ...
  const empty = <T>(populated_by_phase: 1|2|3|4|5|6|7, data: T) => ({
    ...baseEnvelope,
    is_populated: false,
    populated_by_phase,
    data,
  ...
  const populated = <T>(populated_by_phase: 1|2|3|4|5|6|7, data: T) => ({
    ...baseEnvelope,
    is_populated: true,
    populated_by_phase,
    data,
  ...
  const pcd: PCD = {
    ...
    current_phase: 1,
    phase_4_gate_crossed: false,
    vault_references: input.vault_references ?? {
      judge_organizer_ids: [],
      winning_solution_ids: [],
      boilerplate_component_ids: [],
    },
    sections: {
      hackathon_profile:                  populated(1, input.hackathon_profile),
      judge_intelligence:                 empty(1, {}),
      active_judging_rubric:              populated(1, input.default_rubric),
      problem_analysis:                   empty(2, {}),
      competitive_landscape:              empty(2, { premortem_completed: false }),
      user_research:                      empty(2, { research_status: 'not_started' as const }),
      ideation_record:                    empty(3, { archetypes_enforced: true }),
      chosen_direction:                   empty(3, {}),
      scope_definition:                   empty(4, {}),
      technical_direction:                empty(4, {}),
      product_brief:                      empty(4, {}),
      technical_implementation_document:  empty(4, {}),
      demo_script_and_backup_plan:        empty(4, {}),
      pitch_brief:                        empty(6, {}),
      judge_objection_map:                empty(6, {}),
      qa_prep_document:                   empty(6, {}),
      debrief_record:                     empty(7, {}),
      external_context_notes:             empty(1, ''),
    },
    phase_gate_log: [],
    addenda: [],
  };

  validatePCD(pcd);  // throws on schema failure; defensive guard against drift
  return pcd;
}
```

The `populated()` and `empty()` helpers exist only for compactness; they are local to the file and not exported. The full `data` payloads in the table above are inlined here exactly as written — there is no second source of truth.  

#### 2.7.5 Tests for Milestone 1  

Three tests are sufficient for v1.5. Add more only when a real failure mode appears.  

1. **`createEmptyPCD passes Ajv validation`** — call with minimal valid inputs (a stub hackathon profile with only `hackathon_name` set, and a stub rubric with one criterion at `weight: 1.0` and one `judge_question`); assert the returned PCD validates.  
2. **`createEmptyPCD initializes archetypes_enforced to true`** — assert `pcd.sections.ideation_record.data.archetypes_enforced === true`. This is the regression test that protects the §2.6 three‑place‑alignment decision.  
3. **`createEmptyPCD is deterministic when seeded`** — call twice with the same `now` and `newId` stubs; assert the two outputs are deeply equal. This catches accidental nondeterminism (`Date.now()`, `Math.random()`, captured‑but‑mutated fresh array literals) before it becomes a debugging nightmare during reconciliation testing.  
...
The following prompt definitions are added in v1.5. The pre‑existing Phase 3 prompts (SCAMPER, JTBD, Brainwriting, Differentiation Stress Test, Downselection) are **retained unchanged** — they continue to run as Steps 3b–3d (and the existing downstream steps) before Divergent Candidates.  
...
// Addendum Protocol — post‑Phase 4 gate scope change.
{
  id: 'addendum_protocol',
  phase: 5,                                 // triggered from Phase 5 (Build) or later, via Addendum Protocol Interface
  display_name: 'Scope Change Addendum',
  recommended_external_llm: 'claude',
  role_assignment: 'You are a senior engineer producing a tightly scoped addendum to a locked TID. The original TID is canon and is NOT modified. Your output annotates it. You write the smallest correct addendum that captures the change, its impact, and the revised do‑not‑build list. You do not rewrite the TID. You do not expand the scope. You do not soften the change.',
  context_selectors: [
    { section: 'technical_implementation_document', required: true },
    { section: 'chosen_direction', required: false },
    { section: 'scope_definition', required: false },
  ],
  vault_query_hints: null,
  task_specification_template: '...produce an addendum: Change Summary, Impacted Parts, Updated Acceptance Criteria, Revised Do‑Not‑Build list, Risks Introduced...',
  runtime_inputs: [
    {
      name: 'build_state_snapshot',
      kind: 'textarea',
      label: 'Current build state (Cursor export)',
      placeholder: 'Paste the markdown snapshot of the current build state from Cursor...',
      required: true,
      max_length: 100000,
      injects_into: 'task_specification',
    },
    {
      ...
      placeholder: 'Describe what you want to add, modify, or remove...',
      required: true,
      max_length: 5000,
      injects_into: 'task_specification',
    },
    {
      ...
      kind: 'textarea',
      label: 'Justification',
      placeholder: 'Why is this change necessary? (min 80 characters)',
      required: true,
      max_length: 2000,
      injects_into: 'task_specification',
    },
  ],
  target_intake_schema_path: 'addenda',         // appends to pcd.addenda[]
  produces_intake_id: 'addendum_intake',
  // Not Grill‑Me eligible — addenda are direct surgical annotations
}
```
...
The `'if_user_provided'` rule is the inclusion gate for `external_context_notes`. The section is included only when `is_populated === true` AND `data` (the bare string) is non‑empty (i.e., `data.length > 0`). It is the only rule whose source is a section the user authors directly rather than the system populates via intake.  
...
| Addendum Protocol                   | `technical_implementation_document` (required), `chosen_direction` (optional), `scope_definition` (optional)                                                                                                                                                        | —                                                                                                                                  |  
...
If Grill‑Me mode is active, also surface an "Export Handoff Package" button:
  ...
  without Startup managing live chat history.
    │
    ▼
Log generation in Phase Gate Log: a `prompt_generated` event.
  The log entry’s `payload` contains:
    { prompt_id, timestamp, included_vault_ids, grill_me_mode, runtime_inputs_used }
```
...
#### Phase 4 ordering enforcement  

The Prompt Generator surfaces Phase 4 prompts in this order:  

1. Step 4a — Design Grill (optional but recommended; required Grill‑Me when run).  
2. Step 4b — Product Brief.  
3. Step 4c — Technical Implementation Document.  
4. Step 4d — Demo Script & Backup Plan.  

The three document prompts (Product Brief, TID, Demo Script) read `chosen_direction` from context (§4.3). Since Design Grill writes adjustments back into `chosen_direction.data` via `apply_adjustments` (§6.2), running Design Grill *after* a document prompt means that document was generated against pre‑grilled scope.  

This is **not** enforced as a hard precondition the way Step 3e is, because Design Grill is genuinely optional — the workflow doc identifies it as an opt‑in stress test, not a required step. Instead, the Prompt Generator card for each of the three document prompts surfaces a one‑time **"Skip Design Grill" acknowledgment** the first time the user generates one of them within this phase without having run Design Grill first. The acknowledgment is a single inline checkbox immediately above the "Generate Prompt" button, with the copy:  

> *"I'm generating documents without first running the Design Grill. I understand the documents will reflect un‑stress‑tested scope."*  

Once acknowledged, the checkbox does not re‑appear within this phase for any of the three document prompts in the same project. The acknowledgment is recorded as a single `phase_4_design_grill_skipped` event in the Phase Gate Log with payload `{ first_document_prompt_id, timestamp }` (one event per project, not one per prompt).  

If Design Grill is run *after* one or more documents have already been generated, the staleness propagation algorithm (§5.3) marks the affected documents stale through the normal mechanism — `chosen_direction` was updated, the documents depend on it, they go stale. The user resolves them through the standard regenerate / confirm‑valid / manual‑edit flow. No special‑casing.  
...
Every Grill‑Me generation appends a `grill_me_session_initiated` log entry with payload `{ prompt_id, mode, timestamp }`.  

On successful intake of the Resolved Summary, a `grill_me_session_resolved` entry is appended with payload `{ prompt_id, intake_id, timestamp }`.  

If the user dismisses a Design Grill adjustment row during the commit step, a `grill_me_adjustment_dismissed` entry is logged with payload `{ prompt_id, intake_id, field_path, proposed_value, timestamp }`.  

The Seed Challenge verdict (`adopt_as_seed | refined | rejected`) and Design Grill confirmed adjustments are also reflected in their respective intake writes per §6.  
...
| 2     | `problem_analysis`, `competitive_landscape`                                                                                    | `competitive_landscape.data.premortem_completed === true`                                                                                                                                                                                                                            | `problem_analysis.data.differentiation_angles.length >= 1`                                                                                                                                                          | **No** (the only hard block in the system) |  
...
| 4     | `product_brief`, `technical_implementation_document`, `demo_script_and_backup_plan`, `scope_definition`, `technical_direction` | TID has `screens` array non‑empty; demo script has at least one of `backup_plan.screenshots_required`, `backup_plan.screen_recording_segments`, or `backup_plan.live_failure_handling_script` non‑empty                                                                                 | Phase 4 review checklist completed; **scope lock explicitly confirmed** (typed phrase, not a button)                                                                                                                | No                                         |  
...
// Derived constants (used by staleness propagation)
...
// Invert the graph once at module load
...
function invertGraph(graph: Record<string, string[]>): Record<string, string[]> {
  ...
  for (const [section, deps] of Object.entries(graph)) {
    for (const dep of deps) {
      inverted[dep] = inverted[dep] ?? [];
      inverted[dep].push(section);
    }
  }
  return inverted;
}
...
function transitiveClosure(start: string, graph: Record<string, string[]>): Set<string> {
  ...
  visited.delete(start); // transitive closure of dependents, not including self
  return visited;
}
```
...
Form requires:
  ...
  - Description of proposed change
    │
    ▼
System generates Addendum Prompt (see PromptDefinition in §4.2) for external LLM
    │
    ▼
User runs externally, pastes back
    │
    ▼
System extracts via the addendum intake schema (§6.2, `addendum_intake`)
    │
    ▼
...
type IntakeDefinition = {
  ...
  commit_strategy: 'direct' | 'apply_adjustments' | 'append'; // how the extracted object reaches target_pcd_path.
...
**`commit_strategy` semantics.**  

- `'direct'`: the object Flash extracts is proposed as the value of `target_pcd_path` (one section, one diff).  
- `'apply_adjustments'`: Flash extracts a list of adjustments; each adjustment is committed individually against `target_pcd_path` + the adjustment’s `field_path`. Used by `design_grill_intake`.  
- `'append'`: the extracted object is appended to the array at `target_pcd_path` (e.g., `addenda`). Used by `addendum_intake` and any future log‑shaped intakes.  

**The `Adjustment` shape (used by `apply_adjustments`).** When `commit_strategy` is `'apply_adjustments'`, Flash extracts an object whose top‑level shape is `{ adjustments: Adjustment[]; interrogation_summary: string }`. The two required fields on the intake (`required_fields: ['adjustments', 'interrogation_summary']`) correspond to exactly these. Each `Adjustment` is:  

```ts
type Adjustment = {
  field_path: string;            // dotted path within the section identified by
                                 // IntakeDefinition.target_pcd_path. Examples:
                                 //   'user_rationale'
                                 //   'key_differentiators[0]'
                                 //   'premortem_overlap_check.overlap_reasoning'
                                 // Resolved at apply time as
                                 // `${target_pcd_path}.${field_path}`.
  proposed_value: unknown;       // the new value at field_path. Type is whatever the
                                 // schema at the resolved path declares — string,
                                 // number, array, object. Validated against the
                                 // destination schema fragment before the diff
                                 // preview row renders. Schema‑invalid adjustments
                                 // are surfaced as Tier‑2 partial rows the user
                                 // must edit or dismiss; they are never silently
                                 // applied.
  rationale: string;             // why this change emerged from the Grill‑Me
                                 // dialogue. Surfaced in the diff preview row
                                 // and, on apply, persisted into
                                 // user_overrides[].note on the parent section.
};
```

The diff preview renders one row per adjustment, each with its own Apply / Dismiss control. **Apply** writes `proposed_value` to the resolved path, appends a `user_overrides[]` entry on the parent section (`{ field_path, edited_at: now(), note: rationale }`), and runs `propagateStaleness(parentSection)` (§5.3) the same way any other section update would — so a Design Grill that touches `chosen_direction.data.user_rationale` correctly stales `scope_definition`, `technical_direction`, and the Phase 4 documents downstream of it. **Dismiss** appends a `grill_me_adjustment_dismissed` event to the Phase Gate Log with payload `{ prompt_id, intake_id, field_path, proposed_value, timestamp }` (§4.7) and writes nothing to the section.  

**Where `chosenDirectionAdjustmentsSchema` lives.** This is the JSON Schema that encodes the `{ adjustments: Adjustment[]; interrogation_summary: string }` shape. It is passed as `target_schema_fragment` for `design_grill_intake` and lives in `frontend/src/lib/intake/schemas/chosenDirectionAdjustmentsSchema.ts`, alongside any future auxiliary intake schemas. It is **not** placed in `pcd_schema.json` because it describes a transient extraction shape (what Flash returns at intake time), not a durable PCD section. The same pattern applies to any future intake that produces patches against an existing section rather than a wholesale section payload.  

**`post_extract_validators` semantics.** JSON Schema sometimes can’t express a rule that holds for one intake but not another targeting the same path. The canonical case in v1.5 is `associated_persona` on Q&A entries: the schema (`Section_QAPrep`) allows `null` because the standard `intake_phase_6_qa_prep` permits it, but `judge_qa_drill_intake` forbids it. The validators run after schema validation, before the diff preview. A `'flag'` severity routes the intake into Tier 2 (partial review); a `'fail'` severity routes to Tier 3 (manual entry). The vocabulary is deliberately narrow: only `'no_null_at'` exists in v1.5; new rule kinds are added when a real case appears.  

#### v1.5 intake registry additions  

The pre‑existing intakes for SCAMPER, JTBD, Brainwriting, Differentiation Stress Test, and Downselection are **retained unchanged**. The additions below correspond to the new prompts:  
...

{
  ...
  extraction_prompt_template:
    ...
    'are populated by the app post‑extract before schema validation runs (see §6.2 post‑extract enrichment).',
...

{
  ...
  extraction_prompt_template:
    ...
    'intake. On commit, the user chooses (in the diff preview UI) whether the result ' +
    'REPLACES or SUPPLEMENTS the existing qa_prep_document.',
...
// Addendum Protocol intake
{
  ...
  extraction_prompt_template:
    ...
    'The remainder (addendum_id, created_at, user_justification, build_state_snapshot) comes from ' +
    'the Addendum form inputs, not from Flash extraction.',
...
Same as before: assign ULID `id`, set `is_remix = false`, `remix_source_ids = []`.  

#### Addendum intake specific handling  

The addendum form supplies `addendum_id` (mint a ULID), `created_at` (now()), `user_justification` and `build_state_snapshot` (from runtime inputs). Flash extracts only `content_markdown` (the body of the addendum) and related fields if the LLM produces them — the intake merges form inputs with extracted content to assemble the full `TIDAddendum` object before appending.  
...
The frontend’s `extract.ts` transforms the proxy response into `FlashResult` before processing.  

After Flash returns:  

```
function processIntake(result: FlashResult, def: IntakeDefinition): IntakeOutcome {
  // Flash error or schema validation failure
  ...
  // Run per‑intake post‑extract validators
...
The user clicks "Apply Changes" to commit. The diff preview cannot be skipped.  

When a section already has content (e.g., the user is re‑running an intake on a section that was previously populated), the diff shows replacement explicitly. For `judge_qa_drill_intake`, the user explicitly chooses Replace or Supplement before commit; for other intakes in v1.5, the default is replace. Merge‑on‑conflict logic for non‑QA intakes is deferred to a later stretch goal.  

### 6.6 Intake calls the Flash proxy  

Flash is never called from the browser directly. The Cloudflare Worker proxy receives the intake request, attaches the API key, calls Flash, and returns the result.  
...
Cloudflare Worker
  ...
  - Return { data: <parsed inner data JSON>, raw: <full Flash response>, error?: <msg> }
       │
       ▼
Browser (extract.ts) → parse proxyResp.data → FlashResult
       │
       ▼
processIntake → three‑tier outcome
       │
       ▼
...
The user‑customized rubric JSON is stored in the `settings` Dexie table under key `default_rubric_override`. When present, this overrides the bundled markdown parse for newly‑created projects. “Reset to bundled default” clears this key.  
...
│   │   │   │   ├── schemaToTemplate.ts
│   │   │   │   └── templates/       # task specification templates per phase (ports of PROMPTS.md content)
│   │   │   ├── intake/
...
- **No cloud sync.**  
- **No multi‑user.**  
- **No CI/CD specified** (Cloudflare Pages auto‑deploys on push to main).  
...
- **Phase 4:** the prompt cards are surfaced in the order Design Grill → Product Brief → TID → Demo Script (§4.5 "Phase 4 ordering enforcement"). Generating any of the three document prompts without first running Design Grill surfaces a one‑time inline "Skip Design Grill" checkbox acknowledgment immediately above the Generate Prompt button (logged once per project as `phase_4_design_grill_skipped`). The body grows a "Scope Lock Confirmation" panel with a typed-phrase confirmation (the user types "lock scope" exactly to confirm). The Advance CTA is replaced by "Cross the Phase 4 Gate" with a permanent-action warning
...
> **Note on the Rubric Mapping Intake at M1.** §2.7 `createEmptyPCD` expects `default_rubric` to already be the resolved output of the Rubric Mapping Intake when the user pasted official criteria into the New Project Setup textarea (§9.4). The intake itself is not in §6.2's v1.5 additions because it is pre‑existing in the workflow, but the rest of the intake registry doesn't fill out until Milestone 3. Two ways to satisfy this dependency in M1, in order of preference:
>
> - **Recommended — defer the field.** Ship M1's New Project Setup with the **Judging Criteria** textarea visibly disabled and a tooltip reading "Coming in next build — paste official criteria from Settings once available." Re‑enable the textarea in M2 when the Worker proxy and the first IntakeDefinition land. Until then, every M1 project is created with `rubric_source: 'default'`. The user can edit the rubric directly in Settings JSON if they need to override.  
> - **Alternative — pull the proxy forward.** Add the Cloudflare Worker proxy (`/api/extract`) and a single Rubric Mapping IntakeDefinition entry to M1. The rest of the proxy + intake pipeline (Phase 2 registry stubs, three‑tier outcome handler, diff preview) still lands in M2 as planned. Choose this path only if you expect to ship M1 to a real hackathon before M2 is done; otherwise the deferral is cleaner and keeps M1 scope tight.  
